...
public class MetadataGenerator extends JFrame {
    ...
    public MetadataGenerator(@NotNull Project project, @NotNull String tagName, @Nullable Map<TagRule, ?> predefinedData,
        ...
        System.out.println("Got predefinedData map:"); // TODO remove sout
    ...
    private LinkedHashMap<TagRule, Object> build(){
        ...
        System.out.println("Built map:"); // TODO remove sout
...
//            System.out.println("[0] Запускаем для таблицы " + tag + " : " + value.size());
//            for (Map<TagRule, Object> tagRuleObjectMap : value) {
//                for (Map.Entry<TagRule, Object> tagRuleObjectEntry : tagRuleObjectMap.entrySet()) {
//                    System.out.println(tagRuleObjectEntry.getKey().getTagName() + " : " + tagRuleObjectEntry.getValue());
//                }
//            }
...
//        System.out.println(getOffsetStr(offset) + "[1] makePreviewPartFromList, offset=" + offset);
        if ((list != null && !list.isEmpty())){
            for (Object object : list) {
//                System.out.println(getOffsetStr(offset) + "[2] makePreviewPartFromList: добавляем открывающий тег " + parentTagName);
...
//                            System.out.println(getOffsetStr(offset) + "[3] Значение " + tagRule.getTagName() + " это список, запускаем цикл еще раз");
                            makePreviewPartFromList(offset + 1, lst, tagRule, sb);
                        } else if (mapEntry.getValue() instanceof String s) {
                            if (Utils.isNotBlank(s)){
//                                System.out.println(getOffsetStr(offset) + "[3] Значение " + tagRule.getTagName() + " непустое, добавляем " + mapEntry.getValue());
...
//                                System.out.println(getOffsetStr(offset) + "[3] Значение " + tagRule.getTagName() + " пустое, добавляем пустой тег");
                                addComment(sb, tagRule.getTagTooltip(), offset + 1);
...
//                System.out.println(getOffsetStr(offset) + "[2] makePreviewPartFromList: добавляем закрывающий тег " + parentTagName);
                sb.append(getOffsetStr(offset)).append("</").append(parentTagName).append(">\n");
            }
        } else if (addEmptyTagsCheckbox.isSelected()){
//            System.out.println(getOffsetStr(offset) + "[2] makePreviewPartFromList: добавляем пустой тег " + parentTagName);
            sb.append(getOffsetStr(offset)).append("<").append(parentTagName).append("/>\n");
        }
    }
